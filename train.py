...
trainset = ListDataset(root='./data/images',
                       list_file='./data/coco_train_3.txt', train=True, transform=transform, input_size=800)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=4, shuffle=True, num_workers=8, collate_fn=trainset.collate_fn)

testset = ListDataset(root='./data/images',
                      list_file='./data/coco_test_3.txt', train=False, transform=transform, input_size=800)
...
def train(epoch):
    ...
    if (epoch+1)%10==0:
        torch.save(net.module.state_dict(),'./checkpoint/param_{}.pth'.format(epoch+1))
